...
class HeroController extends Controller
{
    ...
    public function store_or_update($request){
        ...
        if($request->hasFile('image')){
            $image = $this->upload_file($request->file('image'),HERO_AVATAR_PATH);
            if(!empty($request->old_image)){
                $this->delete_file($request->old_image,HERO_AVATAR_PATH);
            }
        }
        ...
        // }
        return Hero::updateOrCreate(['id'=>$request->update_id],$collection->all());
    }
    ...
    public function index(Request $request){
        ...
        if($request->ajax()){
            ...
            return DataTables::eloquent($getData)
                ...
                ->addColumn('image', function($row){
                    if ($row->image != null) {
                        $attach = '<img src="'.url('uploads/'.HERO_AVATAR_PATH.$row->image).'" width="50px">';
                    }else{
                        $attach = '';
                    }
                    return $attach;
        ...
        $this->setPageTitle('Hero','Hero List');
        $breadcrumb = ['Hero'=>''];
        return view('backend.heroes.index', ['breadcrumb'=>$breadcrumb]);
    }


    public function updateOrStore(HeroFormRequest $request){
        if ($request->ajax()) {
    ...
    public function bulkDelete(Request $request){
        if ($request->ajax()) {
            $result = Hero::whereIn('id',$request->ids)->select('image')->get();
            if($result){
                foreach ($result as $value) {
                    $this->delete_file($value->image,HERO_AVATAR_PATH);
                }
                Hero::destroy($request->ids);
