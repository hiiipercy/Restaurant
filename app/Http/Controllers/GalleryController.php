...
class GalleryController extends Controller
{
    ...
    public function store_or_update($request){
        ...
        // }
        return Gallery::updateOrCreate(['id'=>$request->update_id],$collection->all());
    }
    ...
    public function index(Request $request){
        ...
        if($request->ajax()){
            $getData = Gallery::orderBy('id','desc');
        ...
        $this->setPageTitle('Gallery','Gallery List');
        $breadcrumb = ['Gallery'=>''];
   
        return view('backend.gallery.index', ['breadcrumb'=>$breadcrumb]);
    }


    public function updateOrStore(GalleryRequest $request){
        if ($request->ajax()) {
    ...
    public function delete(Request $request){
        if ($request->ajax()) {
            $result = Gallery::find($request->id);
            if($result){
                $result->delete();
    ...
    public function bulkDelete(Request $request){
        if ($request->ajax()) {
            $result = Gallery::whereIn('id',$request->ids)->select('image')->get();
            if($result){
