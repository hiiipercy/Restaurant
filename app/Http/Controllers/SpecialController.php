...
class SpecialController extends Controller
{
    ...
    public function store_or_update($request){
        ...
        $section_name = 'SpecialSection';
        $image = $request->old_image;
        if($request->hasFile('image')){
            $image = $this->upload_file($request->file('image'),SPECIAL_AVATAR_PATH);
            if(!empty($request->old_image)){
                $this->delete_file($request->old_image,SPECIAL_AVATAR_PATH);
            }
        }
        ...
        // }
        return Special::updateOrCreate(['id'=>$request->update_id],$collection->all());
    }
    ...
    public function index(Request $request){
        ...
        if($request->ajax()){
            $getData = Special::orderBy('id','desc');
            return DataTables::eloquent($getData)
                ...
                ->addColumn('image', function($row){
                    if ($row->image != null) {
                        $attach = '<img src="'.url('uploads/'.SPECIAL_AVATAR_PATH.$row->image).'" width="50px">';
                    }else{
                        $attach = '';
                    }
                    return $attach;
        ...
        $this->setPageTitle('Special','Special List');
        $breadcrumb = ['Special'=>''];
   
        return view('backend.Special.index', ['breadcrumb'=>$breadcrumb]);
    }


    public function updateOrStore(SpecialRequest $request){
        if ($request->ajax()) {
    ...
    public function delete(Request $request){
        if ($request->ajax()) {
            $result = Special::find($request->id);
            if($result){
                $result->delete();
    ...
    public function bulkDelete(Request $request){
        if ($request->ajax()) {
            $result = Special::whereIn('id',$request->ids)->select('image')->get();
            if($result){
                foreach ($result as $value) {
                    $this->delete_file($value->image,SPECIAL_AVATAR_PATH);
                }
                Special::destroy($request->ids);
