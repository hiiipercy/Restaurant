        ...
        <div class="box box-solid">
            <div class="box-header with-border">
                <div class="box-tools pull-right">
                    <button type="button" class="btn btn-sm btn-primary rounded-0" onclick="showFormModal('New Hero', 'Save')"><i class="fa fa-plus fa-sm"></i> Add Hero</button>
                </div>
            </div>
            <div class="box-body">
                <table class="table table-bordered table-hover table-striped" id="heroes-datatable">
                    <thead>
                        <th>
...
@include('backend.heroes.update_or_store')
@endsection
@push('scripts')
    <script>
        table = $('#heroes-datatable').DataTable({
            processing: true,
            serverSide: true,
            responsive: true,
            ...
            ajax: {
                url: "{{ route('app.heroes.index') }}",
                type: "GET",
                dataType: "JSON",
                data: function(d) {
                    d._token = _token;
                    d.search = $('input[title="search_here"]').val();
                },
            },
            columns: [
                {data: 'bulk_check'},
        ...
        $(document).on('click', '#save-btn', function () {
            ...
            let url = "{{ route('app.heroes.update-or-store') }}";
            ...
            if (id) {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if(data.data.image){
                            var image = "{{ asset('uploads/'.HERO_AVATAR_PATH)}}/"+data.data.image;
        ...
        $(document).on('click','.change_status', function(){
            ...
            var url = "{{ route('app.heroes.status-change') }}"
        ...
        $(document).on('click', '.delete_data', function () {
            ...
            let url = "{{ route('app.heroes.delete') }}";
        ...
        function multi_delete(){
            ...
            if(ids.length == 0){
                ...
                let url = "{{ route('app.heroes.bulk-delete') }}";
